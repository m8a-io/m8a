...
describe("HashService", () => {
  ...
  describe("getPasswordWithSalt", () => {
    ...
    it("should create a password and salt combined string with a password and salt input", async () => {
      ...
      const passWithSalt = await service.getPasswordWithSalt(
        tempPassword,
        salt
      );
      const passWithSaltParts = passWithSalt.split(":");
    ...
    it("should throw an error, if the given salt is too small.", async () => {
      ...
      await expect(
        service.getPasswordWithSalt(tempPassword, saltTooSmall)
      ).rejects.toEqual(
        Error("Salt is too small. Must be at least 24 characters long.")
      );
    });
    ...
    it("should create a hashed password", async () => {
      const hashSpy = jest
        ...
        .mockResolvedValueOnce(
          "$argon2id$v=19$m=4096,t=3,p=1$MuuJhFIytvTeU2JTHd6t8A$mWYVCRQhdUnFpV5dmjmK8Kr61ERYRTQhplZD7tIjmjE"
        );
  ...
  describe("verifyPassword", () => {
    ...
    it("should return false if the password with salt hash is invalid against the stored hash", async () => {
      ...
      const wrongPassWithSalt =
        "thisIsTheWrongPassWord:B5kQNRsGPi8cKAXLQ11Cag==";
    ...
    it("should throw an error, if the password with salt string is malformed", async () => {
      ...
      await expect(
        service.verifyPassword(badPassWithSalt, hashedPassword)
      ).rejects.toEqual(Error("Password with Salt string is malformed."));
    });
    ...
    it("should throw an error, if the password with salt string is malformed: hash is too small", async () => {
      ...
      const hashedPassword = "someStringToFakeAsHash";
      await expect(
        service.verifyPassword(passWithSaltTooSmall, hashedPassword)
      ).rejects.toEqual(
        Error("Salt is too small. Must be at least 24 characters long.")
      );
    });
    ...
    it("should throw an error, if the password from the database is malformed", async () => {
      await expect(
        service.verifyPassword(passWithSalt, hashedPassword.substring(1))
      ).rejects.toEqual(
        Error(
          "Validation of password failed with error: TypeError: pchstr must contain a $ as first char."
        )
      );
    });
  });
