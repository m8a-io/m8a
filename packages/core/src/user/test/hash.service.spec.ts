...
describe('HashService', () => {
  ...
  describe('getPasswordWithSalt', () => {
    ...
    it('should throw an error, if the given salt is too small.', async () => {
      ...
      await expect(service.getPasswordWithSalt(tempPassword, saltTooSmall)).rejects.toEqual(
        Error('Salt is too small. Must be at least 24 characters long.')
      )
    })
  })
  ...
  describe('verifyPassword', () => {
    ...
    it('should throw an error, if the password with salt string is malformed', async () => {
      ...
      await expect(service.verifyPassword(badPassWithSalt, hashedPassword)).rejects.toEqual(
        Error('Password with Salt string is malformed.')
      )
    })
    ...
    it('should throw an error, if the password with salt string is malformed: hash is too small', async () => {
      const passWithSaltTooSmall = 'thisIsSomePassWord:wKhsW!si=='
      const hashedPassword = 'someStringToFakeAsHash'
      await expect(service.verifyPassword(passWithSaltTooSmall, hashedPassword)).rejects.toEqual(
        Error('Salt is too small. Must be at least 24 characters long.')
      )
    })

    it('should throw an error, if the password from the database is malformed', async () => {
      await expect(service.verifyPassword(passWithSalt, hashedPassword.substring(1))).rejects.toEqual(
        Error('Validation of password failed with error: TypeError: pchstr must contain a $ as first char.')
      )
    })
  })
})
