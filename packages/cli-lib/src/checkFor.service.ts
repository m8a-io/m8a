...
export class CheckForService {
  ...
  public docker (): Boolean {
    ...
    const runnerDocker = this.runnerService.getSpawnOutput('docker', ['version', '--format', `'{{.Server.Version}}'`])
    const runnerDockerCompose = this.runnerService.getSpawnOutput('docker compose', ['--version'])

    if (runnerDocker && runnerDockerCompose) {
      const dockerVersion = runnerDocker[1].toString().replace(/(\r\n|\n|\r)/gm, '').replace(/'/g, '')
  ...
  public nerdctl (): Boolean {
    ...
    if (runnerNerdctl) {
      const nerdctlVersion = runnerNerdctl[1].toString().replace(/(\r\n|\n|\r)/gm, '').replace(/'/g, '')
  ...
  public nodeJS (): Boolean {
    ...
    const nodeVersion = runner[1].toString().replace(',', '').replace('v', '').replace(/(\r\n|\n|\r)/gm, '')
