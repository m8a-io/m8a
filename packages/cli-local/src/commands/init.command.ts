...
export class InitCommand implements CommandRunner {
  ...
  async run(input: string[], options: Record<string, any>): Promise<void> {
    ...
    if (input.length > 0) {
      this.logService.error(
        "You've entered something I unfortunately don't understand. Please try again."
      );
      process.exit(1);
    }

    if (options.reset) {
      const doInit = await this.inquirer.ask<{ init: string }>(
        "init",
        undefined
      );
    ...
    this.logService.log(
      "Attempting to add the initial system user to the database."
    );
  ...
  @Option({
    flags: "-r, --reset",
    description:
      "Resets the dev databases, should they not be empty. Use with care!",
  })
