...
@Command({
  name: "up",
  description: `The "up" command will either start the necessary docker containers for your local m8a dev platform or, if starting for the first time, it will also help you initialize a development environment to develop with the m8a platform locally. The counterpart command is "m8a down".`,
})
  ...
  private dockerUp(): void {
    ...
    if (hasDocker || hasNerdctl) {
      ...
      this.logService.error(
        "Sorry. Rancher Desktop needs to be installed and/ or running."
      );
      this.logService.addLine();
      process.exit(1);
    }
