...
export class CheckForService {
  ...
  public docker(): Boolean {
    ...
    const runnerDockerCompose = this.runnerService.getSpawnOutput(
      "docker compose",
      ["--version"]
    );
    ...
    if (runnerDocker && runnerDockerCompose) {
      ...
      this.logService.success(
        `You have Docker available: version ${dockerVersion}`
      );
  ...
  public nerdctl(): Boolean {
    ...
    const runnerNerdctl = this.runnerService.getSpawnOutput("nerdctl", [
      "--version",
    ]);
  ...
  public nodeJS(): Boolean {
    ...
    if (nodeMajorVersion >= 12) {
      this.logService.success(
        `Your Node is good to go: version ${nodeVersion}`
      );
      this.logService.addLine();
      return true;
    } else {
  ...
  public packageManager(): Boolean {
    ...
    if (
      ...
      this.logService.success(
        `Your available package manager(s) is/are good to go.`
      );
      this.logService.addLine();
      return true;
    } else {
      this.logService.error(
        "Your package management either needs updating or is missing."
      );
      this.logService.addLine();
      return false;
    }
