  ...
  <q-banner key="banner" rounded class="text-white bg-red">{{ loginErrorMessage }} </q-banner>
      ...
      <q-card-section>
        <q-form class="q-gutter-md">
          <q-input square filled v-model="username" type="email" label="username"  @keydown="loginError = false" />
          <q-input square filled v-model="password" :type="passwordVisible ? undefined : 'password'" label="password" @keydown="loginError = false">
            <template #append>
                <q-icon
  ...
  setup () {
    ...
    const { mutate: login } = useMutation(Login_Mutation,
      () => ({
        ...
        update: (cache, { data: { login: { accessToken, userId } } }) => {
          ...
          // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
            ...
            $q.notify({ color: 'teal', message: 'You are logged in.', icon: 'thumb_up' })
            onDialogOK()
          }
        }
      }))
